...
add_action('ultimate_links', 'ultimate_favicon', 15);
// Load Favicon in Admin Section
add_action('admin_head', 'ultimate_favicon');
/**
...
function ultimate_favicon() {
	...
	if ("0" == $options['disable_favicon']) {
		if (!empty($options['favicon'])) {
			$ultimate_favicon .= '<link rel="shortcut icon" href="'.esc_url($options['favicon']).'" type="image/x-icon" />';
		}
	}
	echo $ultimate_favicon;
}
/****************************************************************************************/
// Load webpageicon in Header Section
add_action('ultimate_links', 'ultimate_webpage_icon', 20);
/**
 * Get the webpageicon Image from theme options
 * display webpageicon
 *
 */
function ultimate_webpage_icon() {
	$ultimate_webpage_icon = '';
	global $ultimate_theme_setting_value;
	$options = $ultimate_theme_setting_value;
	if ("0" == $options['disable_webpageicon']) {
		if (!empty($options['webpageicon'])) {
			$ultimate_webpage_icon .= '<link rel="apple-touch-icon-precomposed" href="'.esc_url($options['webpageicon']).'" />';
		}
	}
	echo $ultimate_webpage_icon;
}
/****************************************************************************************/
...
function ultimate_headercontent_details() { ?>
	...
	if ( ( !$ultimate_socialnetworks = get_transient( 'ultimate_socialnetworks' ) ) && ( 1 == $set_flags ) )  {
			$social_links      	 = array();
			...
			if ($i > 0) {
				...
				set_transient( 'ultimate_socialnetworks', $ultimate_socialnetworks, 86940 );	 
		}
	...
	<div class="header-main" 
		<?php if(!empty($options['background_image']) && ($options['disable_background_image'] == 0)){ ?>style="background-image:url('<?php echo esc_url($options['background_image']);?>');" <?php } ?> >
		<?php }else{ ?>
		<div class="header-main"
			...
			if (is_home() || is_front_page()) {
				...
				if (('' != ultimate_header_title()) || function_exists('bcn_display_list')) { ?>
					<div class="page-title-wrap">
						<div class="container clearfix">
							<h3 class="page-title"><?php echo ultimate_header_title();?></h3><!-- .page-title -->
						<?php
	...
	if (!$ultimate_featured_sliders != empty($options['featured_post_slider'])) {
		...
		while ($get_featured_posts->have_posts()):$get_featured_posts->the_post();
			...
			if ($title_attribute != '' || $excerpt != '') {
				...
				if ($title_attribute != '') {
					$ultimate_featured_sliders .= '<div class="featured-title"><a href="'.get_permalink().'" title="'.the_title('', '', false).'">'.get_the_title().'</a></div><!-- .featured-title -->';
				}
				if ($excerpt != '') {
