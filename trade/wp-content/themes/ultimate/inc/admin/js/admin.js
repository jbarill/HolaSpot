...
jQuery( document ).ready( function() {
	var selected = 0;
	if( jQuery.cookie( 'ultimate_tab' ) ) {
		selected = jQuery.cookie( 'ultimate_tab' );
		jQuery.cookie( 'ultimate_tab', null );
	}
	var tabs = jQuery( '#ultimate_tabs' ).tabs( { selected: selected } );
	jQuery( '#wpbody-content form' ).submit( function() {
		var selected = tabs.tabs( 'option', 'selected' );
		jQuery.cookie( 'ultimate_tab', selected );
	} );
