...
class ultimate_custom_tag_widget extends WP_Widget {
	function ultimate_custom_tag_widget() {
		$widget_ops  = array('classname' => 'widget_custom-tagcloud', 'description' => __('Displays Custom Tag Cloud', 'ultimate'));
		$control_ops = array('width'     => 200, 'height'     => 250);
		parent::WP_Widget(false, $name = __('TH: Custom Tag Cloud', 'ultimate'), $widget_ops, $control_ops);
	}
...
class ultimate_service_widget extends WP_Widget {
	function ultimate_service_widget() {
		$widget_ops  = array('classname' => 'widget_service', 'description' => __('Display Services ( Business Layout )', 'ultimate'));
		$control_ops = array('width'     => 200, 'height'     => 250);
		parent::WP_Widget(false, $name = __('TH: Services', 'ultimate'), $widget_ops, $control_ops);
	}
	function form($instance) {
...
class ultimate_promobox_widget extends WP_Widget {
	function ultimate_promobox_widget() {
		$widget_ops  = array('classname' => 'widget_promotional_bar', 'description' => __('Display PromoBox ( Business Layout )', 'ultimate'));
		$control_ops = array('width'     => 200, 'height'     => 250);
		parent::WP_Widget(false, $name = __('TH: PromoBox', 'ultimate'), $widget_ops, $control_ops);
	}
	function widget($args, $instance) {
		extract($args);
		...
		?>
			<p>
				<label for="<?php echo $this->get_field_id('promotional_img_background');?>">
					...
					<input class="upload-button1" name="<?php echo $this->get_field_name('promotional_img_background'); ?>" type="button" value="<?php esc_attr_e( 'Upload', 'ultimate' ); ?>" />
			</p>
			<p>
...
class ultimate_recent_work_widget extends WP_Widget {
	function ultimate_recent_work_widget() {
		...
		$control_ops = array('width'     => 200, 'height'     => 250);
		parent::WP_Widget(false, $name = __('TH: Featured Recent Work', 'ultimate'), $widget_ops, $control_ops);
	}
	function form($instance) {
		for ($i = 0; $i < 4; $i++) {
			$var            = 'page_id'.$i;
		...
		?>
			<p>
				...
				<textarea class="widefat" rows="10" cols="20" id="<?php echo $this->get_field_id('text');?>" name="<?php echo $this->get_field_name('text');?>"><?php echo $text;
		?></textarea>
		<?php
		for ($i = 0; $i < 4; $i++) {
			?>
	...
	function widget($args, $instance) {
		...
		?>
			<h3 class="custom-gallery-title"><a href="<?php the_permalink();?>" title=""><?php echo $page_title;
		?></a>
			</h3>
...
class ultimate_Widget_Testimonial extends WP_Widget {
	function ultimate_Widget_Testimonial() {
		...
		$control_ops = array( 'width' => 200, 'height' =>250 ); 
		parent::WP_Widget( false, $name = __( 'TH: Testimonial', 'ultimate' ), $widget_ops, $control_ops);
 	}
	function form( $instance ) {
		...
		<p>
			...
			<input class="upload-button1" name="image-add" type="button" value="<?php esc_attr_e( 'Add Image', 'ultimate' ); ?>" />
		</p>
		<p>
			...
			for( $i=0; $i<2; $i++ ) {
				...
				<div class="<?php echo $testimonial_class; ?>">
					...
					<div class="testimonial-content">
						...
						if(!empty($company_link_array[$i])){  echo $company_link_array[$i]; } ?>" title="<?php if(!empty($company_link_array[$i])){ echo $company_link_array[$i]; } ?>" target="_blank"> <?php if(!empty($company_name_array[$i])){ echo $company_name_array[$i]; }?></a> </div>
					</div>
					<?php } ?>
				</div>
			<?php } 
